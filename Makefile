# This Makefile is for the BAS::Plex::Import extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/BAS/Plex/Import.pm]
#     AUTHOR => [q[Adam Spann <aspann@apple.com>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     NAME => q[BAS::Plex::Import]
#     PREREQ_PM => { File::Path=>q[0], File::Copy=>q[0], Video::Filename=>q[0] }
#     TEST_REQUIRES => {  }
...
EXE_EXT = 
FULL_AR = /usr/bin/ar
LD = cc -mmacosx-version-min=10.11.6
...
DIRFILESEP = /
DFSEP = $(DIRFILESEP)
NAME = BAS::Plex::Import
NAME_SYM = BAS_Plex_Import
VERSION = 0.01
...
MAKE = make
FULLEXT = BAS/Plex/Import
BASEEXT = Import
PARENT_NAME = BAS::Plex
DLBASE = $(BASEEXT)
VERSION_FROM = lib/BAS/Plex/Import.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/BAS/Plex/Import.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/BAS/Plex
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/BAS/Plex
...
TO_INST_PM = lib/BAS/Plex/Import.pm

PM_TO_BLIB = lib/BAS/Plex/Import.pm \
	blib/lib/BAS/Plex/Import.pm
...
DISTNAME = BAS-Plex-Import
DISTVNAME = BAS-Plex-Import-0.01
...
manifypods : pure_all  \
	lib/BAS/Plex/Import.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/BAS/Plex/Import.pm $(INST_MAN3DIR)/BAS::Plex::Import.$(MAN3EXT) 
...
clean :: clean_subdirs
	- $(RM_F) \
	  ...
	  *$(LIB_EXT) MYMETA.json \
	  ...
	  core.[0-9][0-9][0-9][0-9][0-9] perlmain.c \
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) '  - '\''Adam Spann <aspann@apple.com>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: BAS-Plex-Import' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '  Video::Filename: 0' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '      "Adam Spann <aspann@apple.com>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "BAS-Plex-Import",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '            "Video::Filename" : "0"' >> META_new.json
	$(NOECHO) $(ECHO) '         }' >> META_new.json
	$(NOECHO) $(ECHO) '      }' >> META_new.json
	$(NOECHO) $(ECHO) '   },' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <AUTHOR>Adam Spann &lt;aspann@apple.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="File::Copy" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="File::Path" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Video::Filename" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/BAS/Plex/Import.pm blib/lib/BAS/Plex/Import.pm 
